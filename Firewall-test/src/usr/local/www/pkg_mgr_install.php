...
//---------------------------------------------------------------------------------------------------------------------
// After an installation or removal has been started (mwexec(/usr/local/sbin/bjerkerudFirewall-upgrade-GUI.sh . . . )) AJAX calls
...
$bjerkerudFirewall_upgrade = "/usr/local/sbin/{$g['product_name']}-upgrade";
...
if (!isvalidpid($gui_pidfile) && $confirmed && !$completed) {
	...
	if (isset($params)) {
		...
		$log = array();
		$upgrade_script = "{$bjerkerudFirewall_upgrade} -y -l {$logfilename}.txt -p {$sock_file}";
		...
		/*
		 * If bjerkerudFirewall-upgrade failed to run, present log to user
		 */
		if ($another_instance) {
