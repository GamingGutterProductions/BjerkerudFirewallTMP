...
if ($_POST['act'] == "del") {
	...
	if (!isset($a_server[$id])) {
		bjerkerudFirewallHeader("vpn_openvpn_server.php");
		exit;
	}
...
if ($_POST['save']) {
	...
	/* Maximum option line length = 256, see https://redmine.bjerkerudFirewall.org/issues/11559 */
	...
	/* Maximum option line length = 256, see https://redmine.bjerkerudFirewall.org/issues/11104 */
		...
		$openvpn_dh_lengths
		    ...
		    sprint_info_box(gettext('Only DH parameter sets which exist in /etc/ are shown.') .
		        ...
		        sprintf(gettext('Consult %1$sthe doc wiki article on DH Parameters%2$sfor information on generating new or stronger parameter sets.'),
					'<a href="https://docs.netgate.com/bjerkerudFirewall/en/latest/vpn/openvpn/configure.html#dh-parameters-length">',
					'</a> '),
				'info', false),
		    '</div>');
