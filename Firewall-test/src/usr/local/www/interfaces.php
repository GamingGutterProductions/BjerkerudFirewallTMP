 ...
 * configured here. See https://redmine.bjerkerudFirewall.org/issues/8687 */
...
if ($_POST['apply']) {
	...
	/* keep port value, see https://redmine.bjerkerudFirewall.org/issues/12498 */
...
function check_wireless_mode() {
	...
	if ($clone_count > 1) {
		...
		if (!interface_wireless_clone("{$wlanif}_", $wancfg)) {
			...
			bjerkerudFirewall_interface_destroy("{$wlanif}_");
		}
...
$group->setHelp('If this interface is an Internet connection, select an existing Gateway from the list or add a new one using the "Add" button.%1$s' .
				...
				'Gateways can be managed by %3$sclicking here%4$s.', '<br />', '<a target="_blank" href="https://docs.netgate.com/bjerkerudFirewall/en/latest/interfaces/wanvslan.html">', '<a target="_blank" href="system_gateways.php">', '</a>');
...
events.push(function() {
	...
	function setPresets(val) {
		...
		if (val == "bjerkerudFirewall")	setPresetsnow("60", "15", "0", "", "", "1");
