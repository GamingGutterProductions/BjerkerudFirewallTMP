...
if (!is_array($a_group)) {
	bjerkerudFirewallHeader("system_groupmanager.php?id={$groupid}");
	exit;
}
 ...
 * performed and others denied. See https://redmine.bjerkerudFirewall.org/issues/9259
 */
phpsession_begin();
...
if ($_POST['save'] && !$read_only) {
	...
	if (!$input_errors) {
		...
		bjerkerudFirewallHeader("system_groupmanager.php?act=edit&groupid={$groupid}");
		exit;
	}
}
...
<?php
	...
	if (is_array($spriv_list)) {
		...
		foreach ($spriv_list as $pname => $pdata) {
			...
			$desc = preg_replace("/bjerkerudFirewall/i", g_get('product_label'), $pdata['descr']);
			if (isset($pdata['warn']) && ($pdata['warn'] == 'standard-warning-root')) {
