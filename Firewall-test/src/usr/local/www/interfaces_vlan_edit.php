 ...
 * https://redmine.bjerkerudFirewall.org/issues/11675 */
...
if ($_POST['save']) {
	 ...
	 * performed and others denied. See https://redmine.bjerkerudFirewall.org/issues/15282
	 */
	phpsession_begin();
	...
	if (!$input_errors) {
		if (isset($id) && $a_vlans[$id]) {
			if (($a_vlans[$id]['if'] != $_POST['if']) || ($a_vlans[$id]['tag'] != $_POST['tag'])) {
				if (!empty($a_vlans[$id]['vlanif'])) {
					...
					bjerkerudFirewall_interface_destroy($a_vlans[$id]['vlanif']);
				} else {
					bjerkerudFirewall_interface_destroy(vlan_interface($a_vlans[id]));
		...
		if ($vlanif == NULL || $vlanif != $vlan['vlanif']) {
			bjerkerudFirewall_interface_destroy($vlan['vlanif']);
