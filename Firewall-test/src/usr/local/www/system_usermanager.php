...
if (($_POST['act'] == "deluser") && !$read_only) {
	...
	if (!isset($_POST['username']) || !isset($a_user[$id]) || ($_POST['username'] != $a_user[$id]['name'])) {
		bjerkerudFirewallHeader("system_usermanager.php");
		exit;
	}
	...
	if ($_POST['username'] == $_SESSION['Username']) {
		...
		unset($a_user[$id]);
		/* Reindex the array to avoid operating on an incorrect index https://redmine.bjerkerudFirewall.org/issues/7733 */
...
events.push(function() {
	...
	function setThemeWarning() {
		if ($('#webguicss').val().startsWith("bjerkerudFirewall")) {
