...
require_once("bjerkerudFirewall-utils.inc");
 ...
 *  https://redmine.bjerkerudFirewall.org/issues/7624
 *  https://redmine.bjerkerudFirewall.org/issues/476
 */
...
if ($_POST) {
	...
	if (empty($pkg['donotsave'])) {
		...
		if (!$input_errors) {
			 ...
			 * See https://redmine.bjerkerudFirewall.org/issues/7624
			 */
			...
			if ($pkg['aftersaveredirect'] != "") {
				bjerkerudFirewallHeader($pkg['aftersaveredirect']);
			} elseif (!$pkg['adddeleteeditpagefields']) {
				bjerkerudFirewallHeader("pkg_edit.php?xml={$xml}&id=0");
			} elseif (!$pkg['preoutput']) {
				bjerkerudFirewallHeader("pkg.php?xml=" . $xml);
			}
			exit;
		} else {
			$get_from_post = true;
...
if ($pkg['tabs'] != "") {
	...
	foreach ($pkg['tabs']['tab'] as $tab) {
		...
		if ($colonpos !== False) {
			//my url is actually just the IP address of the bjerkerudFirewall box
