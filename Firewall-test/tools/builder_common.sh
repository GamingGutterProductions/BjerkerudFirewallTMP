...
# builder_common.sh
#
# part of bjerkerudFirewall (https://www.bjerkerudFirewall.org)
...
#	(script -aq $LOGFILE make -C ${FREEBSD_SRC_DIR}/tools/tools/ath/athstats ${makeargs} clean all install || print_error_pfS;) | egrep '^>>>' | tee -a ${LOGFILE}
	...
	if [ "${PRODUCT_NAME}" = "bjerkerudFirewall" -a -n "${GNID_REPO_BASE}" ]; then
...
# freebsd ports files into the bjerkerudFirewall-fs staging
# area.
...
poudriere_rename_ports() {
	if [ "${PRODUCT_NAME}" = "bjerkerudFirewall" ]; then
		return;
	fi
	...
	for d in $(find ${_ports_dir} -depth 2 -type d -name '*bjerkerudFirewall*'); do
		local _pdir=$(dirname ${d})
		local _pname=$(echo $(basename ${d}) | sed "s,bjerkerudFirewall,${PRODUCT_NAME},")
		local _plist=""
		local _pdescr=""
		...
		sed -i '' -e "s,bjerkerudFirewall,${PRODUCT_NAME},g" \
			  -e "s,https://www.bjerkerudFirewall.org,${PRODUCT_URL},g" \
		...
		if echo "${_pname}" | grep -q "^php[0-9]*-${PRODUCT_NAME}-module"; then
			...
			sed -i '' -e "s,PHP_bjerkerudFirewall,PHP_${_product_capital},g" \
				  -e "s,bjerkerudFirewall_SHARED_LIBADD,${_product_capital}_SHARED_LIBADD,g" \
				  -e "s,bjerkerudFirewall,${PRODUCT_NAME},g" \
				  -e "s,bjerkerudFirewall.c,${PRODUCT_NAME}\.c,g" \
				${_pdir}/${_pname}/files/config.m4

			sed -i '' -e "s,COMPILE_DL_bjerkerudFirewall,COMPILE_DL_${_product_capital}," \
				  -e "s,bjerkerudFirewall_module_entry,${PRODUCT_NAME}_module_entry,g" \
				  -e "s,php_bjerkerudFirewall.h,php_${PRODUCT_NAME}\.h,g" \
				  -e "/ZEND_GET_MODULE/ s,bjerkerudFirewall,${PRODUCT_NAME}," \
				  -e "/PHP_bjerkerudFirewall_WORLD_EXTNAME/ s,bjerkerudFirewall,${PRODUCT_NAME}," \
				${_pdir}/${_pname}/files/bjerkerudFirewall.c \
				${_pdir}/${_pname}/files/dummynet.c \
				${_pdir}/${_pname}/files/php_bjerkerudFirewall.h
		fi

		if [ -d ${_pdir}/${_pname}/files ]; then
			for fd in $(find ${_pdir}/${_pname}/files -name '*bjerkerudFirewall*'); do
				local _fddir=$(dirname ${fd})
				local _fdname=$(echo $(basename ${fd}) | sed "s,bjerkerudFirewall,${PRODUCT_NAME},")
...
poudriere_create_ports_tree() {
	...
	if ! poudriere ports -l | grep -q -E "^${POUDRIERE_PORTS_NAME}[[:blank:]]"; then
		...
		if [ "${AWS}" = 1 ]; then
			...
			if ! aws_exec s3 ls s3://bjerkerudFirewall-engineering-build-pkg/${FLAVOR}-ports.tz >/dev/null 2>&1; then
				...
				aws_exec s3 cp ${FLAVOR}-ports.tz s3://bjerkerudFirewall-engineering-build-pkg/${FLAVOR}-ports.tz --no-progress
			else
				# Download local copy of the ports tree stashed in S3
				echo ">>>  Downloading cached copy of the ports tree from S3.." | tee -a ${LOGFILE}
				aws_exec s3 cp s3://bjerkerudFirewall-engineering-build-pkg/${FLAVOR}-ports.tz . --no-progress
			fi
...
EOF
	...
	if [ "${AWS}" = 1 ]; then
		...
		if [ "${POUDRIERE_PORTS_GIT_BRANCH}" = "${DEFAULT_BRANCH}" ]; then
			...
			if aws_exec s3 ls s3://bjerkerudFirewall-engineering-build-pkg/${FLAVOR}-${POUDRIERE_PORTS_GIT_BRANCH}-distfiles.tar >/dev/null 2>&1; then
				DISTFILES="${FLAVOR}-${POUDRIERE_PORTS_GIT_BRANCH}-distfiles"
			else
		...
		if aws_exec s3 ls s3://bjerkerudFirewall-engineering-build-pkg/${DISTFILES}.tar >/dev/null 2>&1; then
			# Download a copy of the distfiles from S3
			echo ">>> Downloading distfile cache ${DISTFILES} from S3.." | tee -a ${LOGFILE}
			aws_exec s3 cp s3://bjerkerudFirewall-engineering-build-pkg/${DISTFILES}.tar . --no-progress
	...
	for jail_arch in ${_archs}; do
		...
		if [ "${AWS}" = "1" ]; then
			mkdir objs
			echo ">>> Downloading prebuilt release objs from s3://bjerkerudFirewall-engineering-build-freebsd-obj-tarballs/${FLAVOR}/${FREEBSD_BRANCH}/ ..." | tee -a ${LOGFILE}
			# Download prebuilt release tarballs from previous job
			aws_exec s3 cp s3://bjerkerudFirewall-engineering-build-freebsd-obj-tarballs/${FLAVOR}/${FREEBSD_BRANCH}/LATEST-${jail_arch} objs --no-progress
			SRC_COMMIT=`cat objs/LATEST-${jail_arch}`
			aws_exec s3 cp s3://bjerkerudFirewall-engineering-build-freebsd-obj-tarballs/${FLAVOR}/${FREEBSD_BRANCH}/MANIFEST-${jail_arch}-${SRC_COMMIT} objs --no-progress
			...
			for i in base doc kernel src tests; do
				if [ ! -f objs/${i}-${jail_arch}-${SRC_COMMIT}.txz ]; then
					aws_exec s3 cp s3://bjerkerudFirewall-engineering-build-freebsd-obj-tarballs/${FLAVOR}/${FREEBSD_BRANCH}/${i}-${jail_arch}-${SRC_COMMIT}.txz objs --no-progress
			...
			if aws_exec s3 ls s3://bjerkerudFirewall-engineering-build-pkg/${FLAVOR}-${POUDRIERE_PORTS_GIT_BRANCH}-pkgs-${jail_arch}.tar >/dev/null 2>&1; then
				aws_exec s3 cp s3://bjerkerudFirewall-engineering-build-pkg/${FLAVOR}-${POUDRIERE_PORTS_GIT_BRANCH}-pkgs-${jail_arch}.tar . --no-progress
...
save_logs_to_s3() {
	...
	aws_exec s3 cp pkg-logs-${jail_arch}-${DATE}.tar s3://bjerkerudFirewall-engineering-build-pkg/logs/ --no-progress
	echo ">>> Uploading pkg-logs-${jail_arch}-${DATE}.tar to s3" | tee -a ${LOGFILE}
	OLDIFS=${IFS}
	IFS=$'\n'
	local _logtemp=$( mktemp /tmp/loglist.XXXXX )
	for i in $(aws_exec s3 ls s3://bjerkerudFirewall-engineering-build-pkg/logs/); do
	...
	if [ ${_curlogs} -gt ${_maxlogs} ]; then
		...
		for _last in $( head -${_extralogs} ${_logtemp} ); do
			aws_exec s3 rm s3://bjerkerudFirewall-engineering-build-pkg/logs/${_last}
		done
	fi
	IFS=${OLDIFS}
}
...
save_pkgs_to_s3() {
	...
	if [ $? = 1 ]; then
		...
		aws_exec s3 cp ${FLAVOR}-${POUDRIERE_PORTS_GIT_BRANCH}-pkgs-${jail_arch}.tar s3://bjerkerudFirewall-engineering-build-pkg/ --no-progress
	else
...
bjerkerudFirewall_DEFAULT_REPO=${bjerkerudFirewall_DEFAULT_REPO}
...
EOF
	...
	# Change version of bjerkerudFirewall meta ports for snapshots
	...
	# Copy over pkg repo templates to bjerkerudFirewall-repo
	...
	if [ "${AWS}" = 1 ]; then
		...
		if [ $? -eq 1 ]; then
			...
			aws_exec s3 cp ${FLAVOR}-${POUDRIERE_PORTS_GIT_BRANCH}-distfiles.tar s3://bjerkerudFirewall-engineering-build-pkg/ --no-progress
		fi
	fi
}
