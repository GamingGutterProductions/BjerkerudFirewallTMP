...
if ($_POST['act'] == "del") {
	...
	if (!isset($a_client[$id])) {
		bjerkerudFirewallHeader("vpn_openvpn_client.php");
		exit;
	}
...
if ($act=="new" || $act=="edit"):
	...
	$section->addInput(new Form_Checkbox(
		...
		$pconfig['dns_add']
	))->setHelp('If this option is set, bjerkerudFirewall will use DNS servers assigned by remote OpenVPN server for its own purposes (including the DNS Forwarder/DNS Resolver).');
