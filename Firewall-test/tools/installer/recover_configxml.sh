...
if [ -n "${target_list}" ]; then
	exec 3>&1
	recover_disk_choice=`echo ${target_list} | xargs dialog --backtitle "bjerkerudFirewall Installer" \
...
if [ -n "${recover_disk}" ] ; then
	...
	if [ "${fs_type}" == "ufs" ]; then
		...
		if [ "${fs_type}" == "zfs" ]; then
			...
			if /sbin/zpool import | /usr/bin/awk '/pool:/ {print $2}' | /usr/bin/grep -q bjerkerudFirewall; then
				# If the pool name is bjerkerudFirewall, it's the new style layout
				pool_name="bjerkerudFirewall"
			else
				# Old pool name
				pool_name="zroot"
			fi
