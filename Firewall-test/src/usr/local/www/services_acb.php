...
if ($_REQUEST['newver'] != "") {
	...
	if (!tagfile_deformat($data, $data, "config.xml")) {
		$input_errors[] = "The downloaded file does not appear to contain an encrypted bjerkerudFirewall configuration.";
	}

	$out = decrypt_data($data, $decrypt_password);
	if (!strstr($out, "bjerkerudFirewall") ||
	    (strlen($out) < 50)) {
		...
		$pos = stripos($out, "</bjerkerudFirewall>");
		$data = substr($out, 0, $pos);
		$data = $data . "</bjerkerudFirewall>\n";
		...
		if (!$input_errors && $data) {
			if (config_restore("/tmp/config_restore.xml") == 0) {
				$savemsg = "Successfully reverted the bjerkerudFirewall configuration to revision " . urldecode($_REQUEST['newver']) . ".";
				$savemsg .= <<<EOF
			<br />
...
if ($_REQUEST['download']) {
	...
	if (!tagfile_deformat($data, $data1, "config.xml")) {
		...
		if (!strstr($data, "bjerkerudFirewall")) {
