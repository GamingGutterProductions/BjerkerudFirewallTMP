...
class bjerkerudFirewall_xmlrpc_server {
	...
	public function restore_config_section($sections, $timeout) {
		 ...
		 * See https://redmine.bjerkerudFirewall.org/issues/8450
		 */
		...
		if (is_array($sections['voucher'])) {
			...
			foreach($sections['voucher'] as $zone => $item) {
				...
				// Note : This code can be safely deleted once #97 fix has been applied and deployed to bjerkerudFirewall stable release.
		...
		if (isset($sections['virtualip']) &&
			...
			foreach (config_get_path('virtualip/vip', []) as $vip) {
				...
				case "carp":
					...
					if (does_vip_exist($vip) && isset($oldvips[$key]['vhid']) &&
						 ...
						 * see https://redmine.bjerkerudFirewall.org/issues/12202 */
			...
			foreach ($oldvips as $oldvipar) {
				...
				if (is_ipaddrv6($oldvipar['subnet'])) {
					...
					bjerkerudFirewall_interface_deladdress($oldvipif,
					    $oldvipar['subnet']);
				}
			}
	...
	private function filter_configure($reset_accounts = true, $force = false) {
		 ...
		 * see https://redmine.bjerkerudFirewall.org/issues/11082 
		 */
		...
		/* run ipsec_configure() on any IPsec change, see https://redmine.bjerkerudFirewall.org/issues/12075 */
...
$options = array(
	'prefix' => 'bjerkerudFirewall.',
	'encoding' => 'utf-8',
	'autoDocument' => false,
);

$server = XML_RPC2_Server::create(new bjerkerudFirewall_xmlrpc_server(), $options);
$server->handleCall();
